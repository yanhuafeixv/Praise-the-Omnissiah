...
int pca9685_init(const char *i2c_dev, float freq) {
    ...
    // 1. MODE2: 推挽输出 (OUTDRV=1)，清除所有其他位
    if (write_reg(MODE2, 0x04) < 0) goto fail;

    // 2. 进入睡眠，开启自动增量 (AI=1)，同时清除 SLEEP、ALLCALL 等位（bit0 清零！）
    ...
    // 4. 退出睡眠，RESTART + AI，其他位为零（尤其是 ALLCALL=0）
...
void pca9685_set_servo(int channel, int angle) {
    ...
    uint32_t pulse_us = SERVO_MIN_US + (uint32_t)angle * (SERVO_MAX_US - SERVO_MIN_US) / SERVO_ANGLE_MAX;
