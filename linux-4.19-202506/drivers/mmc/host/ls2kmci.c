...
static void finalize_request(struct ls2k_mci_host *host)
{
	...
	writel(readl(host->base + SDIDCON) & ~(0x1 << 19), host->base + SDIDCON);
...
static irqreturn_t ls2k_mci_irq(int irq, void *dev_id)
{
	...
	if (mci_dsta & SDIDSTA_SDIOIRQDETECT) {
		if (mci_imsk & SDIIMSK_SDIOIRQ) {
			writel(SDIIMSK_SDIOIRQ, host->base + SDIINTMSK);
...
#endif
	if (host->app_cmd == MMC_APP_CMD && (cmd->opcode == 13 || cmd->opcode == 51))
              writel(readl(host->base + SDIDCON) | (0x1 << 19), host->base + SDIDCON);
       if (cmd->data && cmd->opcode == 6) 
              writel(readl(host->base + SDIDCON) | (0x1 << 19), host->base + SDIDCON);
