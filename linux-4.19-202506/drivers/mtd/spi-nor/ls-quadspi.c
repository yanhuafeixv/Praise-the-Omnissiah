                                  ...
                                  struct device_node *np)
{
        ...
        u32 width, presc, cs_num, clk_freq, max_rate, bit = 0;
	u8  setb=0,clrb=0;
	...
	if (!of_property_read_u32(np, "clock-frequency", &clk_freq)) {
		clk_freq = DEFAULT_INPUT_FREQ;
		pr_warn("clock-frequency not found, using default %u Hz\n", clk_freq);
	}
        ...
        presc = DIV_ROUND_UP(clk_freq, max_rate);
	if (presc < 2)
		presc = 2;
