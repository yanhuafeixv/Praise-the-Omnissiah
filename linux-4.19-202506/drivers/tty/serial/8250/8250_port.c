...
static unsigned int mem_serial_in(struct uart_port *p, int offset)
{
	...
	if (p->type == PORT_LOONGSON || p->type == PORT_LOONGSON_FRAC)
...
static unsigned int loongson_frac_get_divisor(struct uart_8250_port *up,
					 unsigned int baud,
					 unsigned int *frac)
{
	struct uart_port *port = &up->port;
					      ...
					      unsigned int *frac)
{
	...
	else if (up->port.type == PORT_LOONGSON_FRAC)
		quot = loongson_frac_get_divisor(up, baud, frac);
			       ...
			       unsigned int quot, unsigned int quot_frac)
{
	...
	if (up->port.type == PORT_LOONGSON_FRAC) {
		serial_port_out(port, 0x2, quot_frac);
	}
}
