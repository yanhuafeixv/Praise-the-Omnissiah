...
static int ls_i2c_hw_config(struct ls_i2c_dev *i2c_dev)
{
	...
	if (i2c_dev->speed == LS_I2C_SPEED_STANDARD) {
		val = DIV_ROUND_UP(i2c_dev->input_clk, 100000 * 2);
	} else {
		val = DIV_ROUND_UP(i2c_dev->input_clk, 400000 * 3);
...
static int ls_i2c_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	u32 irq_event, clk_rate, input_clk;
	struct i2c_adapter *adap;
	int ret;
	...
	if (!of_property_read_u32(np, "input-clock-frequency", &input_clk))
		i2c_dev->input_clk = input_clk;
	else
		i2c_dev->input_clk = INPUT_REF_CLK;
